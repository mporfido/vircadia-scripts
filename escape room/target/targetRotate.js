(function () {
	...
	var resetProperties = ["rotation"]

	// rotation: the vector indicating the rotation in degrees
	// singleUse: whether the rotation should be performed only once (true) or reverted at next call
	// rotated: false if the object is in default orientation
	...
	this.performAction = function() {
		rotation = Entities.getEntityProperties(myID, ["rotation"]).rotation;
		eulerAngles = Quat.safeEulerAngles(rotation);
		actionData = getUserData(myID, "action");

		if (actionData.singleUse) {
			if (!actionData.rotated) {
				eulerAngles = Vec3.sum(eulerAngles, actionData.rotation);
				actionData.rotated = true;
			}
		} else {
			if (actionData.rotated) {
				eulerAngles = Vec3.subtract(eulerAngles, actionData.rotation);
				actionData.rotated = false;
			} else {
				eulerAngles = Vec3.sum(eulerAngles, actionData.rotation);
				actionData.rotated = true;
			}
		...
		Entities.editEntity(myID, {rotation: Quat.fromVec3Degrees(eulerAngles) });	
	};
